...
Процедура Расш1_РезультатПриАктивизацииОбластиПосле(Элемент)
    ...
    Элементы.Итог.Заголовок = Сред(Элементы.Итог.Заголовок, 1, 3) + " " + Строка(ОбщийМодульКлиент.РасчетЯчеек(Результат,
        Неопределено).Сумма);
...
Процедура Расш1_ВыгрузитьРеестр(Команда)
    ...
    Если Глоссаб Тогда
        ...
        ИмяФайла = "\\16x-deductor01.one.local\userData$\common\1С\Глоссаб\ДДС\Выгрузки\Контроль реестра Глоссаб";
...
Процедура ЗаполнитьДатыИзБуфераНаСервере(АдресРеестра, ТекстБуфера, ИдентификаторФормы)
    ...
    ТЗ_Тавлинцев.Колонки.Добавить("НомерДокумента", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(100,
    ...
    Пока Индекс <= Строки.Количество()-1 Цикл
        ...
        Попытка
            ...
            Если ТЗ_Тавлинцев.Найти(Номер, "НомерДокумента") <> Неопределено Тогда

                ВызватьИсключение "Ошибка в номере документа: " + Строки[Индекс - 1]
            ...
            ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Обработано: " + Номер);          
...
Процедура Расш1_ОбработатьРеестр(РезультатЗакрытия, ДополнительныеПараметры)
    ...
    Если ДополнительныеПараметры = "Бюджетирование План-Факт" Тогда
        ...
        Оповестить("БюджетированиеПланФакт", ?(РезультатЗакрытия = Неопределено, АдресРеестра, ПолучитьАдресРеестра(РезультатЗакрытия,

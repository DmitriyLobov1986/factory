...
Функция УстановитьОтборИзРеестраНаСервере(СписокБаз)
    ...
    Попытка
        ...
        ТЗ_Реестр_Фильтр = ОбщийМодульСервер.ПолучитьСложныйОтборТЗ(ТЗ_Реестр, "ТипОрганизации", ВидСравнения.ВСписке, ,
            СписокБаз);
		//
...
Процедура Расш1_ЗадолженностиПосле(Команда)
    ...
    ПоляРасшифровки = ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Идентификатор, ДанныеРасшифровки,
        Новый Структура);
    ...
    ОткрытьФорму("Отчет.ЗадолженностиПоТовару.Форма.ФормаОтчета", ПараметрыФормы, ЭтаФорма, Истина,
...
Процедура Расш1_ОтправитьПисьмо(Команда)
    ...
    СписокПолучателей.Добавить("sas@pochtavip.com, konstantin.charchyan@pochtavip.com",
        "Смирнов Александр Станиславович");

    ОповещениеОЗакрытии = Новый ОписаниеОповещения("УстановитьПолучателя", ЭтаФорма,
        ЭтаФорма[ЭтаФорма.ТекущийЭлемент.Имя]);

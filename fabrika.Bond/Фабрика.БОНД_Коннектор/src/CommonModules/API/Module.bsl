...
Функция ПолучитьДанныеПоAPI(База, Параметры, ХранилищеСессий, Сообщение = "") Экспорт
    ...
    ПутьДоHttpСервисаСпараметрами = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(
        ПодключениеHTTP.СтрокаПодключения, Параметры);

    Аутентификация = Новый Структура("Пользователь, Пароль", ПодключениеHTTP.Пользователь, ПодключениеHTTP.Пароль);
    ПараметрыПреобразованияJSON = Новый Структура("ПрочитатьВСоответствие, ИменаСвойствСоЗначениямиДата", Ложь,
        "Период,Дата,День,ОплатитьДо");
    ДопПараметры = Новый Структура("Аутентификация, Заголовки, Таймаут, ПараметрыПреобразованияJSON", Аутентификация,
        ПараметрыСессии.headers, 100, ПараметрыПреобразованияJSON);

    Попытка
        Результат = БОНД_Коннектор.PostJson(Сервер + "/" + ПутьДоHttpСервисаСпараметрами, Параметры, ДопПараметры,
            ПараметрыСессии.ХранилищеСессий[База]);
		//---<>---
    ...
    Исключение
        ...
        Сообщение = Строка(База) + ":" + ТекстСообщения;
        Возврат Неопределено;
    КонецПопытки;

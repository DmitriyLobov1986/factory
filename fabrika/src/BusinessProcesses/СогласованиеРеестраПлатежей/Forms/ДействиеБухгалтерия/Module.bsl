...
Процедура ПрочитатьРеквизитыБизнесПроцесса()
	...
	ЗаданиеВыявленыОшибки = ЗаданиеОбъект.ВыявленыОшибки;
...
Процедура ЗаписатьРеквизитыБизнесПроцесса(ЗадачаОбъект)
	...
	ЗаданиеОбъект.ХранилищеРеестраПлатежей = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресРеестра));
	//конец Дима
	
	////Дима 03.10.2018 11:53:02////Запишем реквизит "Хранилище реестра ТД"
	Если ТипЗнч(РеестрТД) = Тип("ТабличныйДокумент") Тогда
	  ЗаданиеОбъект.ХранилищеРеестраТД = Новый ХранилищеЗначения(РеестрТД);
	КонецЕсли;  
	//конец Дима
	
	////Дима 8/3/2016 10:46:03 AM////Запишем реквизит "Выявлены ошибки"
...
Процедура НаименованиеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура("АдресРеестра, Исполнитель, ТекущийПользователь, РольИсполнителя, УИД", АдресРеестра, Объект.Исполнитель, ТекущийПользователь, Объект.РольИсполнителя, УникальныйИдентификатор);
	
	ОткрытьФорму("Отчет.РеестрПлатежейСогласование.Форма.ФормаОтчета", ПараметрыФормы, ЭтаФорма,,ВариантОткрытияОкна.ОтдельноеОкно,, ОповещениеОЗакрытии);
...
Процедура ЗаписьРеестра(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		ЗаданиеВыявленыОшибки = РезультатЗакрытия.Корректировка;

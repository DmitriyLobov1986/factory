...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЭтоНовый() Тогда
		...
		//Добавлено Лобовым Д.М.
		Попытка
			ХранилищеРеестраПлатежей = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(ДанныеЗаполнения.АдресДетализации));
		Исключение
		КонецПопытки;
		//
...
Процедура ВыполнитьПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	////Дима 8/15/2016 4:49:20 PM////Установим текущую стадию выполнения бизнес-процесса
	СтадияВыполнения = ТочкаМаршрутаБизнесПроцесса;
	//конец Дима
	...
	Для каждого Задача Из ФормируемыеЗадачи Цикл
		...
		////Дима 29.07.2016 14:18:51////Добавил автора задачи в качестве исполнителя
		Если ТочкаМаршрутаБизнесПроцесса.НаименованиеЗадачи = "Исправление ошибок" Тогда
			Задача.Исполнитель = Автор;
		КонецЕсли;	
		//конец Дима
		
		Задача.Наименование = НаименованиеЗадачиДляВыполнения(ТочкаМаршрутаБизнесПроцесса.Имя);
...
Процедура СтартПередСтартом(ТочкаМаршрутаБизнесПроцесса, Отказ)
	...
	////Дима 16.05.2017 13:36:48////Заполним таблицу "История выполнения"
	РезультатВыполненияТочкиВыполнить(Неопределено);
	//конец Дима
...
Функция НаименованиеЗадачиДляВыполнения(ТочкаМаршрута = "")
	
	Если ТочкаМаршрута = "КлиентБанк" Тогда
		Возврат "Отправить платежи" + ?(ВторойРеестр(), " (второй реестр)", "");
	ИначеЕсли ТочкаМаршрута = "ИсправлениеОшибок" Тогда
		Возврат "Исправить ошибки";
	ИначеЕсли Глоссаб Тогда
		Возврат "Реестр базы Заказы"
	Иначе	
...
Функция РезультатВыполненияТочкиВыполнить(Знач ЗадачаСсылка)
	...
	////Дима 16.05.2017 13:37:35////Для самого бизнесс-процесса
	Если ЗадачаСсылка = Неопределено Тогда
		НоваяЗапись = ИсторияВыполнения.Добавить();
		НоваяЗапись.ДатаИсполнения = Дата;
		НоваяЗапись.Исполнитель = Автор;
		НоваяЗапись.РольИсполнителя = "Автор";
	...
	ЗадачаДанные = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗадачаСсылка, 
		"РезультатВыполнения,ДатаИсполнения,Исполнитель,РольИсполнителя");
	...
	////Дима 26.07.2016 10:13:34////Заполним ТЧ "История выполнения"
	НоваяЗапись = ИсторияВыполнения.Добавить();
	НоваяЗапись.ДатаИсполнения = ЗадачаДанные.ДатаИсполнения;
	НоваяЗапись.Исполнитель = ЗадачаДанные.Исполнитель;
	НоваяЗапись.РольИсполнителя = ЗадачаДанные.РольИсполнителя;
	НоваяЗапись.Выполнено = Выполнено;
...
//////////////////////////////////////////////////////////////////////////////////////
//ПРОЦЕДУРЫ И ФУНКЦИИ ЛОБОВА ДМ


Процедура ВыявленыОшибкиПроверкаУсловия(ТочкаМаршрутаБизнесПроцесса, Результат)
	
	Результат = ВыявленыОшибки;
	
КонецПроцедуры


Функция ВторойРеестр()
	
	Перем Время;
	
	////Дима 07.06.2018 11:11:45////Проверим текущее время и установм номер реестра
	Время = Дата - НачалоДня(Дата);
	ВторойРеестр = ?(Время >= 57600, Истина, Ложь);
	//конец Дима
	
	Возврат ВторойРеестр;
...
Процедура ВыборВарианта1ОбработкаВыбораВарианта(ТочкаВыбораВарианта, Результат)
	...
	Если Глоссаб Тогда
		Результат = ТочкаВыбораВарианта.Варианты.Глоссаб;
		
	ИначеЕсли ВторойРеестр Тогда
		Результат = ТочкаВыбораВарианта.Варианты.ВторойРеестр;
		
	Иначе Результат = ТочкаВыбораВарианта.Варианты.ПервыйРеестр;		
	КонецЕсли;

...
Функция ОбработатьПараметрыЗапуска(ЗначениеПараметраЗапуска, ПараметрыЗапуска) Экспорт
	...
	////Дима 24.09.2014 10:33:00////Запуск процедуры создания документов оплат зарубежным поставщикам
	Если ЗначениеПараметраЗапуска = "ЗАРУБЕЖ" тогда
		ТаможняСервер.СозданиеОплатЗарубеж(ПараметрыЗапуска);

...
Процедура ОбновлениеИнформационнойБазы() Экспорт
	...
	////Дима 05.07.2016 16:32:36////Инициализировать собственные роли
	РольОбъект = Справочники.РолиИсполнителей.Бухгалтер.ПолучитьОбъект();
	ЗаблокироватьДанныеДляРедактирования(РольОбъект.Ссылка);
	РольОбъект.ИспользуетсяБезОбъектовАдресации = Истина;
	РольОбъект.ИспользуетсяСОбъектамиАдресации = Истина;
	РольОбъект.ТипыОсновногоОбъектаАдресации = Неопределено;
	РольОбъект.Записать();
	//конец Дима

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    АдресСхемыКомпоновкиДанных.Вставить("ДляРедактирования", ПоместитьВоВременноеХранилище(СхемаДляРедактирования,
        УникальныйИдентификатор));
    АдресСхемыКомпоновкиДанных.Вставить("ДетализацияРасходов", ПоместитьВоВременноеХранилище(СхемаДетализацияРасходов,
        УникальныйИдентификатор));
    АдресСхемыКомпоновкиДанных.Вставить("Основная", ПоместитьВоВременноеХранилище(СхемаОсновная,
        УникальныйИдентификатор));
КонецПроцедуры
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
    ...
    Иначе
        СписокОрганизаций.Добавить(ВыбранноеЗначение);
...
#Область РезультатДляРедактирования
...
Процедура РезультатДляРедактированияПриИзмененииСодержимогоОбласти(Элемент, Область, Получатель = Неопределено)
    ...
    Если ЗаписатьВРегистр = "Распределить по счетам" Тогда
        ...
        ОткрытьФорму("Обработка.ЗаполнениеРегистраДДС.Форма.Форма", П, ЭтаФорма, , , , Оповещение,
...
Процедура РезультатДляРедактированияОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
    ...
    КонецЕсли;
    ОбработкаРасшифровкиКД.ВыбратьДействие(Расшифровка, ВыполненноеДействие, ПараметрВыбранногоДействия,
    ...
    ИначеЕсли ВыполненноеДействие = "Выбрать получателя" Тогда
		//
        РасшифровкаОбласти = Новый Структура("РасшифровкаМассив, ДанныеРасшифровки",
            ПолучитьРасшифровкиВыделеннойОбласти(
			ЭтаФорма[Элемент.Имя]), ДанныеРасшифровкиДляРедактирования);
        ПараметрыФормы = Новый Структура("ЗакрыватьПриВыборе, Расшифровка", Истина, РасшифровкаОбласти);
		//
        ОткрытьФорму("РегистрСведений.НастройкаПолейОтправки.ФормаСписка", ПараметрыФормы, ЭтаФорма, , , , ,
            РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
    ИначеЕсли ВыполненноеДействие = "Добавить организации" Тогда
		//
        РасшифровкаОбласти = Новый Структура("РасшифровкаМассив, ДанныеРасшифровки",
            ПолучитьРасшифровкиВыделеннойОбласти(
			РезультатДляРедактирования), ДанныеРасшифровкиДляРедактирования);
        ПараметрыФормы = Новый Структура("ЗакрыватьПриВыборе, Расшифровка, ДобавитьОрганизации", Ложь,
            РасшифровкаОбласти, Истина);
		//
        ОткрытьФорму("РегистрСведений.НастройкаПолейОтправки.ФормаСписка", ПараметрыФормы, ЭтаФорма, , , , ,
            РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
    ...
    ИначеЕсли ВыполненноеДействие = "ДДС" Тогда
		...
		//
        ОткрытьФорму("РегистрСведений.ДДС.ФормаСписка", ПараметрыФормы, ЭтаФорма, , , , ,
            РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
    ...
    ИначеЕсли ТипЗнч(ПараметрВыбранногоДействия) = Тип("НастройкиКомпоновкиДанных") Тогда
        ОтработатьРасшифровку(Схема, Новый ОписаниеОбработкиРасшифровкиКомпоновкиДанных(АдресРасшифровки, Расшифровка,
...
&НаКлиенте
Процедура РезультатДляРедактированияОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка,
...
Процедура ОтработатьРасшифровку(Схема, ОписаниеОбработкиРасшифровки)
    ИсточникНастроек = Новый ИсточникДоступныхНастроекКомпоновкиДанных(Схема);
    ОбработкаРасшифровкиКД = Новый ОбработкаРасшифровкиКомпоновкиДанных(ОписаниеОбработкиРасшифровки.Данные,
        ИсточникНастроек);
    НастройкаКомпоновкиДляРасшифровки = ОбработкаРасшифровкиКД.ПрименитьНастройки(
		ОписаниеОбработкиРасшифровки.Идентификатор, ОписаниеОбработкиРасшифровки.ПрименяемыеНастройки);
...
Процедура СформироватьИзВыделеннойОбласти(МассивРасшифровок = Неопределено, АдресРасшифровки = "ДанныеРасшифровки")
    ...
    Если МассивРасшифровок <> Неопределено Тогда
        ...
        КонецЕсли;
        ЗаписатьВыделеннуюОбластьНаСервере(МассивРасшифровок, ЭтаФорма[АдресРасшифровки], СписокОрганизаций,
            СписокПериодов);
    КонецЕсли;  
	//=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>
    ...
    ИначеЕсли ВидДетализации = 0 Тогда
        РедактироватьНаСервере(СписокОрганизаций, СписокПериодов, "ДляРедактирования", Тип(
    ...
    Иначе
        ...
        Если НастройкаКомпоновкиДляРасшифровки = Неопределено Тогда
            РедактироватьНаСервере(СписокОрганизаций, СписокПериодов, "ДляРедактирования", Тип(
				"ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"), "ДляДетализацииРасходов");
            РедактироватьНаСервере(СписокОрганизаций, СписокПериодов, "ДетализацияРасходов", Тип(
				"ГенераторМакетаКомпоновкиДанных"));
			//=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>
...
&НаСервере
Процедура РедактироватьНаСервере(Организации, Период, ИмяСхемы, ТипГенератораМакета, ВариантОтчета = "Основной",
	...
	//=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>
    Если (ИмяСхемы = "ДляРедактирования" Или ИмяСхемы = "ДетализацияРасходов") И ВариантОтчета = "Основной" Тогда
        Схема = ОтчетОбъект.ПолучитьМакет(ИмяСхемы);
        КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(Схема));
		
		//Определим настройки компоновки данных
        Если Не НастройкаДляРедактирования Или (ИмяСхемы = "ДляРедактирования" И ДетализацияРасходов) Тогда
            НастройкиКомпоновки = ХранилищеПользовательскихНастроекОтчетов.Загрузить("ОтчетДДС", ИмяСхемы
                + ".ОбщиеНастройки");
        ...
        Если ИмяСхемы = "ДляРедактирования" Тогда
            ...
            Если Период.Количество() > 1 Тогда
                ...
                Если ОтборПериод Тогда
                    ФиксированныйОтбор = НастройкиКомпоновки.Отбор.Элементы.Добавить(Тип(
        ...
        ИначеЕсли ИмяСхемы = "ДетализацияРасходов" Тогда
            ...
            ЭлементОтбора.ЛевоеЗначение = ПолеОтбора;
            ЭлементОтбора.ВидСравнения = ?(Запад, ВидСравненияКомпоновкиДанных.ВСписке,
            ...
            Попытка
                ...
                ВнешнийНаборДанных.Вставить("РеестрПлатежей", РеестрПлатежей.Скопировать(
					Новый Структура("ОтборСуществующихПП", Ложь)));
				//
				...
				//
            Исключение
                ОбщийМодульСервер.ДобавитьСоообщениеВмассив( ,
                    "Загрузите платежи с помощью обработки ""Загрузка платежей""");
                РезультатДляРедактирования.Очистить();
                Возврат;
            КонецПопытки;
        КонецЕсли;
    ...
    Если ТипГенератораМакета = Тип("ГенераторМакетаКомпоновкиДанных") Тогда
        ...
        Если НастройкаКомпоновкиДляРасшифровки = Неопределено Тогда
            ...
            ЭтаФорма["ДанныеРасшифровки" + СтрЗаменить(ТаблицаВывода, "Результат", "")] = ПоместитьВоВременноеХранилище(
				НовыеДанныеРасшифровки, ЭтаФорма.УникальныйИдентификатор);
        Иначе
...
Процедура ДДС(Команда)
    ...
    //
    РасшифровкаОбласти = Новый Структура("РасшифровкаМассив, ДанныеРасшифровки", ПолучитьРасшифровкиВыделеннойОбласти(
        Результат), ДанныеРасшифровки);
    ПараметрыФормы = Новый Структура("Расшифровка, КлючПользовательскихНастроек", РасшифровкаОбласти, "Пустые");
    //
    ОткрытьФорму("РегистрСведений.ДДС.ФормаСписка", ПараметрыФормы, ЭтаФорма, , , , ,
        РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаписатьВыделеннуюОбластьНаСервере(МассивРасшифровок, ДанныеРасшифровки, СписокОрганизаций, СписокПериодов)
    ...
    Для Каждого Идентификатор Из МассивРасшифровок Цикл
        РасшифровкаВыделеннойОбласти.Добавить(ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Идентификатор,
...
Процедура ПолучитьРасшифровку(Расшифровка, Сумма, ОстатокНаКонец, Получатель)
    СтруктураРасшифровки = ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Расшифровка, ЭтаФорма["ДанныеРасшифровки"
        + СтрЗаменить(ТекущийЭлемент.Имя, "Результат", "")], Новый Структура);
...
Процедура СделатьЗаписьВРегистр(Получатель)
    ...
    ИначеЕсли Получатель <> Неопределено Тогда
        ЗаписатьВРегистр = "Распределить по счетам";
    Иначе
        ОбработкаДДС.ПроверитьНаСоответствиеРегиструДДС();
...
Процедура РедактироватьРеквизиты(Реквизит = Неопределено, Отказ = Ложь)
    ...
    Для Каждого Область Из ВыделенныеСтроки Цикл
        МассивРасшифровок.Добавить(Область.Расшифровка);
    КонецЦикла;
    ПолучитьРасшифрокуВМассивНаСервере(МассивРасшифровок, ДанныеРасшифровкиДляРедактирования, АдресДетализации,
        ЗначениеУстановки);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПолучитьРасшифрокуВМассивНаСервере(МассивРасшифровок, ДанныеРасшифровкиДляРедактирования, АдресДетализации,
    ЗначениеУстановки = Неопределено)
    Перем Ошибки;
    ...
    Для Счётчик = 0 По МассивРасшифровок.ВГраница() Цикл
        ...
        СтруктураРасшифровки = ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Расшифровка,
            ДанныеРасшифровкиДляРедактирования, Новый Структура);
        Если Счётчик = 0 И ЗначениеУстановки = Неопределено Тогда
            ...
            Для Каждого Запись Из НайденныеЗаписи Цикл
                Если ТипЗнч(ЗначениеУстановки) = Тип("Структура") Тогда
                    ...
                    Если Запись.Организация <> ЗначениеУстановки.СчетОрганизации.Владелец Тогда
                        ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "РезультатДляРедактирования",
                            "Счёт принадлежит другой организации!!!");
                        //ВызватьИсключение "Счёт принадлежит другой организации!!!";
                    КонецЕсли;
                    Если Запись.СчетОрганизации.Валюта <> ЗначениеУстановки.СчетОрганизации.Валюта И Не Запись.СчетОрганизации
                        = Справочники.БанковскиеСчета.ПустаяСсылка() Тогда
                        ВызватьИсключение "Разные валюты счетов";
                    КонецЕсли;
                    Если Не ДДС_Сервер.ПроверитьСчётНаПолучателя(ЗначениеУстановки.СчетОрганизации,
                        Запись.СтранаПолучателя) Тогда
                        ВызватьИсключение "Выбранный счёт не подходит для оплаты контрагенту!!!" + Символы.ПС
                            + Запись.СсылкаБухгалтерия;
                    КонецЕсли;  
                    
                    //
                    Запись.Дата = ?(ЗначениеУстановки.Свойство("Период"), ЗначениеУстановки.Период, НачалоДня(
                        ТекущаяДата()));
                    //
...
Процедура УстановитьСтруктуруОтбора(СтруктураРасшифровки, Знач ТЗдетализация)
    ...
    Для Каждого Элемент Из СтруктураРасшифровки Цикл
        ...
        КонецЕсли;
        Если ТЗдетализация.Колонки.Найти(Элемент.Ключ) = Неопределено Или Элемент.Значение = Null Тогда
...
Процедура ЗаписатьОстатокНаКонецНаСервере()
    ...
    ТЗ.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0,
...
Процедура НастройкаОтборов(Команда)
    ...
    ПереключитьМакетРедактирования();
...
Процедура ИнициализироватьНастройкуДляРедактирования()
    ...
    Если Не НастройкаДляРедактирования Тогда
        ...
        КомпоновщикНастроекРезервный.Инициализировать(
			Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемыКомпоновкиДанных.Основная));
        КомпоновщикНастроекРезервный.ЗагрузитьНастройки(Отчет.КомпоновщикНастроек.Настройки);
        КомпоновщикНастроекРезервный.ЗагрузитьПользовательскиеНастройки(
        ...
        Отчет.КомпоновщикНастроек.Инициализировать(
			Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемыКомпоновкиДанных[ИмяСхемы]));
        НастройкиКомпоновки = ХранилищеПользовательскихНастроекОтчетов.Загрузить("ОтчетДДС", ИмяСхемы + ".Настройки");
        ПользовательскиеНастройки = ХранилищеПользовательскихНастроекОтчетов.Загрузить("ОтчетДДС", ИмяСхемы
            + ".ПользовательскиеНастройки");
        Если НастройкиКомпоновки = Неопределено Тогда
            НастройкиКомпоновки = ПолучитьИзВременногоХранилища(
				АдресСхемыКомпоновкиДанных[ИмяСхемы]).ВариантыНастроек["Основной"].Настройки;
        КонецЕсли;
    ...
    Иначе
        ...
        Отчет.КомпоновщикНастроек.ЗагрузитьПользовательскиеНастройки(
			КомпоновщикНастроекРезервный.ПользовательскиеНастройки);
        Отчет.КомпоновщикНастроек.Восстановить();
    КонецЕсли;

    НастройкаДляРедактирования = Не НастройкаДляРедактирования;
    Элементы.ФормаЗаписьНастроек.Доступность = НастройкаДляРедактирования;
    Элементы.НастройкиОтчетаДляРедактирования.Доступность = НастройкаДляРедактирования;
    Элементы.ФормаНастройкаОтборов.Заголовок = ?(НастройкаДляРедактирования, "Выйти", "Настроить");
    Элементы.ИзменитьВариантДляРедактирования.Доступность = НастройкаДляРедактирования;
    Элементы.СменитьВидМакетаРедактирования.Доступность = Не НастройкаДляРедактирования;
...
Процедура ЗаписьНастроекНаСервере()
    ИмяСхемы = ?(Не ДетализацияРасходов, "ДляРедактирования", "ДетализацияРасходов");
    Попытка
        ХранилищеПользовательскихНастроекОтчетов.Сохранить("ОтчетДДС", ИмяСхемы + ".Настройки",
            Отчет.КомпоновщикНастроек.Настройки);
        ХранилищеПользовательскихНастроекОтчетов.Сохранить("ОтчетДДС", ИмяСхемы + ".ПользовательскиеНастройки",
            Отчет.КомпоновщикНастроек.ПользовательскиеНастройки);
        ХранилищеПользовательскихНастроекОтчетов.Сохранить("ОтчетДДС", ИмяСхемы + ".ОбщиеНастройки",
...
&НаКлиенте
Процедура ПереключитьМакетРедактирования()

    Если Элементы.МакетРедактирования.Поведение = ПоведениеОбычнойГруппы.Обычное Или НастройкаДляРедактирования Тогда

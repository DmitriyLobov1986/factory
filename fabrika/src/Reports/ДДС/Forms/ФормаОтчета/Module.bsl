...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
    Если ТипЗнч(ВыбранноеЗначение) <> Тип("СправочникСсылка.Организации") Тогда
        РезультатДляРедактированияПриИзмененииСодержимогоОбласти(ТекущийЭлемент, ЭтаФорма[ТекущийЭлемент.Имя].ТекущаяОбласть,
            ВыбранноеЗначение);
    Иначе
...
Процедура РезультатДляРедактированияОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
    ...
    КонецЕсли;
    ОбработкаРасшифровкиКД.ВыбратьДействие(Расшифровка, ВыполненноеДействие, ПараметрВыбранногоДействия, ДоступныеДействия,
        ...
        ИначеЕсли ИмяСхемы = "ДетализацияРасходов" Тогда
            ...
            Попытка
                ...
                Если ТЗостатков.Колонки.Найти("Контрагент") = Неопределено Тогда
                    ТЗОстатков.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(10,
...
Процедура ЗаписатьВыделеннуюОбластьНаСервере(МассивРасшифровок, ДанныеРасшифровки, СписокОрганизаций, СписокПериодов)
    ...
    Для Каждого Идентификатор Из МассивРасшифровок Цикл
        РасшифровкаВыделеннойОбласти.Добавить(ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Идентификатор, ДанныеРасшифровки,
            Новый Структура));
    КонецЦикла;
...
Процедура ПолучитьРасшифровку(Расшифровка, Сумма, ОстатокНаКонец, Получатель)
    СтруктураРасшифровки = ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Расшифровка, ЭтаФорма["ДанныеРасшифровки" + СтрЗаменить(
        ТекущийЭлемент.Имя, "Результат", "")], Новый Структура);
...
Процедура ПолучитьРасшифрокуВМассивНаСервере(МассивРасшифровок, ДанныеРасшифровкиДляРедактирования, АдресДетализации,
    ...
    Для Счётчик = 0 По МассивРасшифровок.ВГраница() Цикл
        ...
        Если ТипЗнч(Расшифровка) <> Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
            Продолжить;
        КонецЕсли;
        СтруктураРасшифровки = ОбщийМодульСервер.ПолучениеРасшифровкиВСтруктуру(Расшифровка, ДанныеРасшифровкиДляРедактирования,
            Новый Структура);
            ...
            Для Каждого Запись Из НайденныеЗаписи Цикл
                Если ТипЗнч(ЗначениеУстановки) = Тип("Структура") Тогда
                    ...
                    Если Не ДДС_Сервер.ПроверитьСчётНаПолучателя(ЗначениеУстановки.СчетОрганизации, Запись.СтранаПолучателя) Тогда
                        ТекстОшибки =  "Выбранный счёт не подходит для оплаты контрагенту!!!" + Символы.ПС + Запись.СсылкаБухгалтерия;
                        ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "РезультатДляРедактирования", ТекстОшибки);
                    ...
                    Если Запись.СчетОрганизации.Валюта <> ЗначениеУстановки.СчетОрганизации.Валюта И Не Запись.СчетОрганизации
                        ...
                        Если СтрНайти(Запись.КонтрагентНаименование, Строка(ЗначениеУстановки.СчетОрганизации.Валюта)) = 0 Тогда
                            ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "РезультатДляРедактирования",
                                "Разные валюты счетов");
                            ВызватьИсключение "Разные валюты счетов";
                        КонецЕсли;

                        КурсИсх = РегистрыСведений.КурсыВалют.СрезПоследних( , Новый Структура("Валюта", Запись.СчетОрганизации.Валюта));
                        КурсНовый = РегистрыСведений.КурсыВалют.СрезПоследних( , Новый Структура("Валюта",
                            ЗначениеУстановки.СчетОрганизации.Валюта));
                        Запись.Сумма = Окр(Запись.Сумма * КурсИсх[0].Курс / КурсНовый[0].Курс, 2);
                        Запись.Валюта = ЗначениеУстановки.СчетОрганизации.Валюта;
...
Процедура ЗаписатьОстатокНаКонецНаСервере()
    ...
    ТЗ.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 0, ДопустимыйЗнак.Неотрицательный)));

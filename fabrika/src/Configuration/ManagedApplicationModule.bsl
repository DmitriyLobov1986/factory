...
Процедура ПередЗавершениемРаботыСистемы(Отказ, ТекстПредупреждения)
	...
	#Если НЕ ВебКлиент Тогда
		Если ПроверкаСистем Тогда	
			ТекстПредупреждения = "Чтобы проверить все системы перед закрытием нажмите Продолжить...";
			ПодключитьОбработчикОжидания("ПроверитьСистемыПередЗавершениемРаботы", 1, Истина);
		Иначе
			ТекстПредупреждения = "Вы подтверждаете закрытие приложения?";
		КонецЕсли;  
	#КонецЕсли
...
#Область ПроверкаСистемПередЗавершениемРаботы
Процедура ПроверитьСистемыПередЗавершениемРаботы() Экспорт
	...
	Если НЕ Отказ Тогда
		//
		ПроверкаСистем = Ложь;
		ПодключитьОбработчикОжидания("УстановитьСтатусПроверкиСистем", 5, Истина);
...
Процедура УстановитьСтатусПроверкиСистем() Экспорт
	ПроверкаСистем = Истина;
КонецПроцедуры	

...
Процедура ПослеЗаписиНаСервере(ID)
	ВнешниеИсточникиДанных.SqlЗатратыНовый.ПересчитатьИндекс("CostItem", ID);
КонецПроцедуры
...
Функция ПолучитьIDНовойЗаписи()
  ...
  Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
                 |	сп_СтатьяРасхода.ID КАК ID
                 |ИЗ
                 |	ВнешнийИсточникДанных.Бюджетирование.Таблица.сп_СтатьяРасхода КАК сп_СтатьяРасхода
                 |
                 |УПОРЯДОЧИТЬ ПО
                 |	ID УБЫВ";

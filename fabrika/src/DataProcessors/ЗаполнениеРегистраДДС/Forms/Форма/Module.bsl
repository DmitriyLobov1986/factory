...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.Расшифровка.Получатель) = Тип("СправочникСсылка.Контрагенты") Тогда
		Если Объект.Расшифровка.Организация.ВалютныйРассчёт
		                                    ИЛИ Объект.Расшифровка.Получатель.ВалютныйРассчёт Тогда     
...
Процедура ФормулаКонвертацииНажатие(Элемент)
	...
	МассивСлагаемых = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элементы.ФормулаКонвертации.Заголовок, " =");
...
Процедура ОбновитьДанные()
	...
	Для Счётчик = 0 По МассивСлов.Количество() - 1 Цикл
		Если СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(МассивСлов[Счётчик]) Тогда
			МассивСлов[Счётчик] = "Элементы.РаспределениеПоСчетам.ТекущиеДанные[" + Символ(34) + МассивСлов[Счётчик] + Символ(34) + "]";
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьСтроки(Обработка)
	...
	Объект.РаспределениеПоСчетам.Свернуть("Владелец,Организация,СчетКонтрагента,ДоговорыКонтрагентов,Валюта,Курс,ДеньВДень,НомерПлатежа,НомерВБухгалтерии ", "Сумма,ВалютнаяСумма");
...
Процедура ОбновитьДинамическийСписок()
	...
	ИсторияОтправки.Параметры.УстановитьЗначениеПараметра("Контрагент", Элементы.РаспределениеПоСчетам.ТекущиеДанные.Владелец);
	ИсторияОтправки.Параметры.УстановитьЗначениеПараметра("СчетКонтрагента", Элементы.РаспределениеПоСчетам.ТекущиеДанные.СчетКонтрагента);
...
Процедура НастройкаРазмераФормы(Команда)       
	...
	ОткрытьФорму("Обработка.ЗаполнениеРегистраДДС.Форма.Форма", ПараметрыФормы, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
КонецПроцедуры
//конец Дима
...
Процедура РедактироватьВалютныйПлатеж(Команда)
	...
	Иначе
		...
		ДанныеЗаполнения.Вставить("ДоговорыКонтрагентов", Элементы.РаспределениеПоСчетам.ТекущиеДанные.ДоговорыКонтрагентов);
				
		П.Вставить("ЗначенияЗаполнения", ДанныеЗаполнения); 
		
	КонецЕсли;	
	
	ОткрытьФорму("Документ.ТаможняРассчёты.Форма.ФормаДокумента", П, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

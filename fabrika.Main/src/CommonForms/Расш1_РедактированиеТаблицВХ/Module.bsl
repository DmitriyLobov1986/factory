...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьТаблицуДляРедактирования"
		                       И ЭтоАдресВременногоХранилища(Параметр) Тогда
...
Функция ОтобразитьТаблицуЗначенийНаФорме(знач ТаблицаЗначений = Неопределено) Экспорт
	...
	КонецЕсли;	
	//
	ТаблицаФормы = Элементы.Вставить(ИмяРеквизитаТаблицыПоУмолчанию, Тип("ТаблицаФормы"), Элементы["ТаблицыРедактирования"], Элементы["ДанныеСтроки"]); 	
	...
	Для каждого ТекКолонка Из ТаблицаЗначений.Колонки Цикл
		...
		МассивРеквизитовДобавить.Добавить(Новый РеквизитФормы(ИмяСоздаваемойКолонкиРеквизита,
														      НовоеОписаниеТипов,
	...
	Если МассивРеквизитовДобавить.Количество() > 0 Или
		                                             МассивРеквизитовУдалить.Количество() > 0 Тогда
...
Функция ПреобразоватьТЗвДерево(ТЗ)
    ...
    Если ПоляГруппировки.Количество() > 0 Тогда
        ...
        Для Каждого ПолеГруппировки Из ПоляГруппировки Цикл
            ...
            Если ОписаниеТипаКолонки.СодержитТип(Тип("Число"))
                                                              И НЕ ПолеГруппировки.Использование Тогда
